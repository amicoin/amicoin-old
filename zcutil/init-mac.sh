...
if [ ! -f "$HOME/Library/Application Support/AMICoin/amicoin.conf" ]; then
    echo "Creating amicoin.conf"
    mkdir -p "$HOME/Library/Application Support/AMICoin/"
    echo "rpcuser=zcashrpc" > ~/Library/Application\ Support/AMICoin/amicoin.conf
    PASSWORD=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    echo "rpcpassword=$PASSWORD" >> "$HOME/Library/Application Support/AMICoin/amicoin.conf"
    echo "Complete!"
fi
